...
Make.com receives the HTTP POST transmitted by the Wix Automation `POST_To_Make_Inlet_Webhook` on form submission. `WH_Wix_Inquiry_To_Make` listens for incoming payloads and passes each bundle to `SF_Make_Lead_To_Salesforce`.
...
Make.com executes the Salesforce Create a Record API call to create the Lead Record with all mapped field values in a single operation.
...
| Writing Module | `SF_Make_Lead_To_Salesforce` — Salesforce Create a Record |
