...
Every Lead Record that enters the Flow is a confirmed B2B submission. The Gatekeeper is enforced at the Wix form layer before any data is transmitted. No qualification logic, qualification fields, or disqualification paths exist in this Flow.
...
**Elements:** `Determine Priority Level` (Decision), `Priority Level High` (Assignment), `Priority Level Medium` (Assignment), `Priority Level Low` (Assignment)
...
**Responsibility:** Determine the final `OwnerId` value by either retaining the Assignment Rule `OwnerId` or overriding it with Sophia Delgado's User ID based on Priority Level.

**Elements:** `Initialize OwnerId (Default)` (Assignment), `Escalate High Priority to Sophia` (Decision), `Escalate OwnerId to Sophia` (Assignment)
...
| `Is Not High` | Default Outcome | Assignment Rule `OwnerId` retained |
...
The Flow contains 26 elements across all four segments.
