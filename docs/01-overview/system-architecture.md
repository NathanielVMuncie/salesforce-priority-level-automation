...
This document describes the architecture of the Céleste Vineyards Lead Priority Level Automation system. It defines each layer of the system, the responsibilities of each layer, the boundaries between them, and the sequence in which they execute.

This document does not define Field-level logic, scoring weights, threshold values, or Flow configuration details. This document establishes the shape of the system — the layers, the sequence, and the handoff points — not the internal logic of any individual component.
...
Wix hosts the public-facing inquiry form through which Leads initiate contact with Céleste Vineyards. The form collects the Field values that will ultimately drive qualification, scoring, and routing decisions in Salesforce.

**Responsibilities:**
- Collect Lead-submitted Field values via the inquiry form
...
- Make.com is in scope for Field mapping and ingestion logic documentation only.
...
|---|---|---|---|
| 1 | Wix | Lead submits inquiry form | Form payload transmitted to Make.com |
...
| 7 | Flow — Escalation | Priority Level evaluated | Priority Level High: OwnerId set to Sophia Delgado. Medium / Low: Regional Queue OwnerId retained. |
...
|---|---|---|
| Wix inquiry form | External | Lead capture |
| Make.com scenario | External | Field normalization and API ingestion |
| Salesforce Lead Object | Salesforce | Record storage and Field container |
| After-Save Flow | Salesforce | Qualification, scoring, priority assignment, and escalation |
| Assignment Rules | Salesforce | Territory-based routing to owner or Queue |
| Queues | Salesforce | Lead holding by regional territory |
