...
| Default Branch | main |
| Author | Nathaniel V. Muncie — Salesforce Administrator candidate |
...
- Priority Level High Leads are routed immediately to the correct owner or escalation Queue without manual intervention
...
1. **Lead capture.** A prospect submits the Céleste Vineyards inquiry form on the Wix platform.
...
7. **Routing.** The assigned Priority Level, combined with territory data, triggers the appropriate Assignment Rule, which routes the Lead to the correct sales representative or Queue.
...
|---|---|---|---|
| Business Type | `Business_Type__c` | 5 | Evaluates structural alignment between the prospect's organization type and Céleste Vineyards' target account profile |
...
## 9. Documentation Category Map

Documentation is organized into categories. Each category produces a set of files that are required inputs for subsequent categories.

| Category | Name | Key Deliverables |
|---|---|---|
| Overview | Root Control Plane | README, project overview, scope boundaries, business objective, system architecture |
| Integration | Integration Boundary | Wix form and automation, key/value mapping, Make.com integration, Salesforce integration |
| Data Model | Data Authority | Field inventory, field dictionary, custom field metadata, formula definitions |
| Architecture | Core Deterministic Logic | Gatekeeper logic, scoring model, scoring logic, priority thresholds, priority assignment |
| Automation Logic | Routing, Escalation, and Ownership | Territorial routing, escalation logic, Assignment Rules, Queue definitions, Flow notes |
| Validation and Evidence | Validation and Evidence | Test scenarios, UAT matrix, test records, expected results, defect log, screenshots |

---

## 10. System Status

This system is fully operational. All automation logic, routing rules, and integration components are built and live in a Salesforce Developer Edition org. Documentation is produced in reverse documentation mode from the operational system. Six live validation records confirm end-to-end pipeline execution across all paths — two Not Qualified Gatekeeper Fail, Priority Level Low, Priority Level Medium, and two Priority Level High with escalation.
