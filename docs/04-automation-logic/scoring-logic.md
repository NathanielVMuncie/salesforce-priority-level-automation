...
Céleste Vineyards | Phase 2 — Core Deterministic Logic
...
| Purchasing Timeline | 2 | `Determine_Purchasing_Timeline_Score` | 4 |
| **Total Maximum** | | | **14** |
...
```
IF Role__c = "Purchasing Manager"       THEN varTotalScore += 5
IF Role__c = "Owner / General Manager"  THEN varTotalScore += 4
IF Role__c = "Buyer"                    THEN varTotalScore += 3
IF Role__c = "Sales Manager"            THEN varTotalScore += 2
IF Role__c = "Marketing Coordinator"    THEN varTotalScore += 1
...
IF Purchasing_Timeline__c = "Mid-Term (1–3 Months)"          THEN varTotalScore += 3
IF Purchasing_Timeline__c = "Long-Term (3–6 Months)"         THEN varTotalScore += 2
IF Purchasing_Timeline__c = "Budget Planning (Future Quarter)"THEN varTotalScore += 2
IF Purchasing_Timeline__c = "Exploratory (No Set Timeline)"  THEN varTotalScore += 1
```
...
|---|---|---|---|
| Purchasing Manager | `Role__c` Equals `Purchasing Manager` | Assignment: Add 5 to `varTotalScore` | 5 |
| Owner / General Manager | `Role__c` Equals `Owner / General Manager` | Assignment: Add 4 to `varTotalScore` | 4 |
| Buyer | `Role__c` Equals `Buyer` | Assignment: Add 3 to `varTotalScore` | 3 |
| Sales Manager | `Role__c` Equals `Sales Manager` | Assignment: Add 2 to `varTotalScore` | 2 |
| Marketing Coordinator | `Role__c` Equals `Marketing Coordinator` | Assignment: Add 1 to `varTotalScore` | 1 |
...
| Mid-Term (1–3 Months) | `Purchasing_Timeline__c` Equals `Mid-Term (1–3 Months)` | Assignment: Add 3 to `varTotalScore` | 3 |
| Long-Term (3–6 Months) | `Purchasing_Timeline__c` Equals `Long-Term (3–6 Months)` | Assignment: Add 2 to `varTotalScore` | 2 |
...
| Exploratory (No Set Timeline) | `Purchasing_Timeline__c` Equals `Exploratory (No Set Timeline)` | Assignment: Add 1 to `varTotalScore` | 1 |
| Default | No match | Routes to End — no Assignment fires | 0 |
...
|---|---|
| `varTotalScore` | Composite accumulated value from Segments 1 and 2 (range: 0–14 for fully matched submissions) |
...
- **Test Record:** Neil Thompson (existing validation record)
- **Input:** `Role__c` = `Purchasing Manager`
- **Expected:** `varTotalScore` incremented by 5 following Segment 1 Business Type score
- **SOQL:** `SELECT Lead_Score__c FROM Lead WHERE LastName = 'Thompson' AND FirstName = 'Neil'`
- **UI:** Lead Record → Details → Lead Score — confirm final score reflects Role contribution

### 6.2 Purchasing Timeline — Short-Term Path

- **Test Record:** Neil Thompson
...
- **Input:** `Business_Type__c` = `Upscale Restaurant` (3 pts), `Role__c` = `Buyer` (3 pts), `Purchasing_Timeline__c` = `Mid-Term (1–3 Months)` (3 pts)
